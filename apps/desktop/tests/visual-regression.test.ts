...
test("visual fixture covers the required state matrix and reduced motion mode", async () => {
  ...
  assert.match(script, /strictPort: true, hmr: false/);
  assert.match(fixture, /层次视觉夹具未完成父 Project 打开/);
...
test("hierarchy controls expose keyboard and accessible state semantics", async () => {
  ...
  // 元件库项目与层级清单均使用原生 button；层级按钮另外暴露当前选中状态。
  assert.match(sidebar, /<button[^>]+class="component-item"/);
  assert.match(sidebar, /:aria-pressed="component\.id === selectedComponentId"/);
...
test("canvas exposes non-color state hooks for ports and wires", async () => {
  ...
  // 悬空只有一个布尔量：端点缺失与两端位宽不再相同都归它，线路外观因此只认它。
  assert.match(canvas, /v-if="!wire\.dangling"/);
