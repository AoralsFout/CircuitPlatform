...
当前可能出现的错误代码包括 `bad_json`、`bad_request`、`invalid_kind`、`invalid_connection`、`invalid_signal`、`invalid_input`、`port_not_found`、`combinational_loop` 和 `unsupported_message`。

## 生命周期约定

- 引擎进程启动后持有一份 `Circuit`；同一进程内的请求共享这份结构。
- 添加元件或连接后会重建 `Simulation` 快照；因此当前版本的结构修改会清空运行时状态。
- 删除元件时，领域层仍保留相关悬空 Connection；本协议尚未暴露删除操作，后续增加时必须保持这一规则。
