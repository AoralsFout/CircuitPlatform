...
在现有工作区骨架上接入真正的元件添加、节点拖动、画布平移、端口连线和检查器属性编辑。连接创建、仿真触发和状态管理由画布控制器或领域层负责，`CircuitNode` 只负责展示和抛出事件。

验收标准：

- 从元件库添加的节点拥有独立编辑器位置，移动不会改变仿真结果；
- 从输出端口拖到输入端口可以创建连接，非法方向、重复输入和不存在端口均显示原因；
- 取消连接不会留下临时线，成功连接后信号状态与引擎结果一致；
- 检查器显示选中对象的名称、类型、端口、连接和可编辑属性；
- 关键操作提供键盘等价路径，并通过前端交互测试和 `pnpm verify`。
