...
- 同一份 Project 只存在一个标签。文档身份是它的规范化绝对路径，因此未保存的 Project 没有身份，不能被引用，也不能成为标签；
...
1. 引擎多文档承载：Electron 主进程的 `EngineClient` 从模块级单例改为按文档管理的进程池，补齐懒启动、按文档的健康检查、关闭回收和崩溃隔离；
...
- 强制结束某份文档的引擎进程，其余文档的仿真不受影响，该文档进入"引擎不可用"状态并可重启；
